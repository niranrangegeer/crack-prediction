# ============================================================
# 多孔发泡材料裂纹预测 - 内存全量缓存极限加速版 (适配256G内存)
# 包含：全量内存预加载、自适应居中裁剪、图像加粗、加权L1 Loss、LSGAN、TTUR、动态曲线
# ============================================================
...
# 强制关闭 OpenCV 内部多线程与 OpenCL 加速，防止打满 CPU
cv2.setNumThreads(0)      
...
class CrackDataset(Dataset):
    def __init__(self, triplets, is_train=True):
        ...
        # 将所有图片一次性读入 256GB 内存中！
        print(f"\n[{'训练集' if is_train else '测试集'}] 正在将 {len(self.triplets)} 组图片加载到内存中，请稍候...")
        for idx in tqdm(range(len(self.triplets)), desc="预加载进度"):
            gf, sf, stf = self.triplets[idx]
            img_g = Image.open(gf).convert('RGB')
            img_s = Image.open(sf).convert('RGB')
            img_st = Image.open(stf).convert('RGB')
            # 在装入内存前直接完成加粗，减少后续计算
            img_st = thicken_status_image(img_st, kernel_size=3, iterations=1)
            self.cache[idx] = (img_g, img_s, img_st)
        print(f"[加载完成] {'训练集' if is_train else '测试集'} 数据已常驻内存！\n")

    def __len__(self): 
        return len(self.triplets)

    def __getitem__(self, idx):
        # 训练时直接从内存字典读取，彻底告别机械硬盘的读写延迟！
        img_g, img_s, img_st = self.cache[idx]
        return apply_transforms(img_g, img_s, img_st, self.is_train)
...
if __name__ == '__main__':
    ...
    # [核心修改] 保持大 Batch Size喂饱GPU，并将多线程设为0（内存读取单线程最快，避免IPC开销）
    BATCH_SIZE = 32  
    NUM_WORKERS = 0  
    ...
    try:
        for epoch in range(START_EPOCH, TOTAL_EPOCHS):
            ...
            for geom_t, sener_t, status_t in pbar:
                ...
                weight_mask = torch.ones_like(status_t).to(DEVICE)
