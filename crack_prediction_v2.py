...
# 新增: 随时停止+断点续训 + 进度条显示 + 内存预加载 + AMP混合精度 + 大批量
# ============================================================
...
class CrackDataset(Dataset):
    def __init__(self, triplets, is_train=True):
        ...
        # 预加载所有图像到内存，消除训练时磁盘 I/O 瓶颈
        desc = f"预加载{'训练' if is_train else '测试'}集图像"
        self._cached = []
        for gf, sf, stf in tqdm(triplets, desc=desc, unit="样本"):
            self._cached.append((
    ...
    def __getitem__(self, idx):
        geom_pil, sener_pil, status_pil = self._cached[idx]
...
if __name__ == '__main__':
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'Training Loss (v2.1, stopped at epoch {len(loss_history)})')
    plt.legend()
