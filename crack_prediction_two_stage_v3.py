# ============================================================
# 多孔发泡材料裂纹预测 — 两步法 v3 (终极加速版)
#
# Stage 1: Geom → Sener    （几何结构 → 应变能场，替代 FEM）
# Stage 2: Geom + Sener → Crack  （几何 + 应变能 → 裂纹）
#
# v3 新增 (基于v2):
#   - 纯Tensor预加载+share_memory_ (v7终极加速方案)
#   - 随机裁剪/翻转为Tensor操作 (抛弃PIL)
# v2 保留:
#   - 裂纹加权系数 50→20 + 加粗核 3→2
#   - 每30epoch输出 + 独立输出文件夹
#   - Batch=32 Workers=8 (3090 24G)
# ============================================================
...
class CrackDataset(Dataset):
    """v3: 纯Tensor预加载 + share_memory_ — 训练时零I/O、零PIL开销"""
    def __init__(self, triplets, is_train=True):
        ...
        print(f"\n[{desc}] 预处理并缓存至内存...")
        for idx in tqdm(range(len(self.triplets)), desc=desc):
            gf, sf, stf = self.triplets[idx]

            # 一次性完成所有预处理：居中裁剪 → 加粗(Status) → resize → Tensor归一化
            img_g  = center_square_crop(Image.open(gf).convert('RGB'))
            img_s  = center_square_crop(Image.open(sf).convert('RGB'))
            img_st = center_square_crop(thicken_status_image(Image.open(stf).convert('RGB'), kernel_size=2, iterations=1))
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            t_st = t_st[:, top:top+IMG_SIZE, left:left+IMG_SIZE]

            # Tensor 高速随机翻转
            if torch.rand(1).item() > 0.5:
                t_g  = torch.flip(t_g,  dims=[2])
                t_s  = torch.flip(t_s,  dims=[2])
                t_st = torch.flip(t_st, dims=[2])
...
if __name__ == '__main__':
    ...
    # ---- 断点续训 ----
    CKPT_PATH = os.path.join(SAVE_DIR, "checkpoint_two_stage_v3.pth")
