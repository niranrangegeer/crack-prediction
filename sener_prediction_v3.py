# ============================================================
# 应变能场预测 v3 — Geom → Sener (残差预测, 纯L1回归)
#
# v3: 预测 Sener - Geom 残差, 输出 = Geom + 残差
#     解决v2模型直接复制Geom输出的作弊问题
...
RUN_OUTPUT_DIR = os.path.join(OUTPUT_ROOT, f"sener_pred_v3_{RUN_TIMESTAMP}")
...
def train_one_epoch(model, train_loader, opt, scaler, criterion):
    ...
    for geom_t, sener_t in train_loader:
        ...
        opt.zero_grad()
        with autocast():
            residual = model(geom_t)           # 预测残差 Sener - Geom
            pred = geom_t + residual            # 输出 = Geom + 残差
...
def save_visual_samples(model, test_loader, epoch, output_dir, device):
    ...
    with torch.no_grad():
        ...
        with autocast():
            sener_pred = geom_t + model(geom_t)  # Geom + 残差
...
if __name__ == '__main__':
    print("=" * 60)
    print("  应变能场预测 v3 — Geom → Sener (残差预测)")
